...
В коротком эпизоде система удержала выбранного игрока на 100.0% sampled-кадров. По движению игрок показал высокоинтенсивное ускорение и активно менял позицию. Близость мяча к игроку зафиксирована на 3.5% видимых кадров, контекст давления рядом с другими игроками — на 78.8%.
...
- `00:03.1` `run_into_space` / `neutral` / `medium`: Peak movement speed: 3.98 body-heights/sec.
- `00:17.7` `carrying_ball` / `neutral` / `low`: YOLO detected the ball near the tracked player; exact pass/dribble outcome is not claimed.
